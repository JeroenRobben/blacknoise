...
| **ActiveResponder** | Fully established session (responder). Both directions of transport data can flow.                                                                                                  | `t_send_r`, `t_recv_r` (transport keys); outgoing nonce counter; sliding window for replay protection; session index; peer session index.   |
